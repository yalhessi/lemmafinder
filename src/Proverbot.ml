...
let scrape_data prelude proverbot fname =
  ...
  let script = proverbot ^ "src/scrape.py " in
  let cmd = python ^ script ^ "--prelude=" ^ prelude ^ " " ^ fname ^ " -P" in
...
let search prelude proverbot fname axiom_opt conjecture_name =
  ...
  let python = "python3 " in
  let script = proverbot ^ "src/search_file.py " in
  let weights_file = proverbot ^ "data/polyarg-weights.dat " in
  let cmd =
    Consts.fmt
      "timeout 30 %s %s --prelude=%s --weightsfile=%s %s %s \
       --no-generate-report --max-proof-time=15 -P -o %s/search-report-%s-%s"
