...
let generate_lfind_file (p_ctxt : proof_context) (conjecture : string)
    (name : string) : string =
  let lfind_file = p_ctxt.dir ^ "/lfind" ^ name ^ ".v" in
  let module_imports =
  ...
  let content =
    Consts.fmt
      "%s%s\n\
       ...
       Admitted.\n\
       QuickChick %s.\n"
      Consts.lfind_declare_module p_ctxt.declarations Consts.quickchick_import
      p_ctxt.namespace p_ctxt.fname module_imports typ_derive conjecture name
  in
  FileUtils.write_to_file lfind_file content;
  lfind_file

let check_validity (conjecture : conjecture) (p_ctxt : proof_context) :
    bool * string list =
...
let validity_stats conjectures p_ctxt =
  ...
  Parmap.parmap ~ncores:n_cores
    (fun c ->
      let is_valid, cgs = check_validity c p_ctxt in
      let g_stat =
        {
          conjecture = c;
          is_valid;
...
let helpful_lemma_stats stats p_ctxt =
  ...
  Parmap.parmap ~ncores:1
    (fun s ->
      if s.is_valid then
        let is_provable =
          Provable.check_lfind_theorem_add_axiom p_ctxt
            s.conjecture.conjecture_name s.conjecture.conjecture_str
        in
        let s = { s with is_provable } in
        s
      else s)
    (Parmap.L stats)
...
let lemma_provable_stats stats p_ctxt =
  ...
  Parmap.parmap ~ncores:1
    (fun s ->
      if s.is_valid then
        let is_prover_provable = Provable.check_provable s.conjecture p_ctxt in
        let time_to_p = int_of_float (Unix.time ()) - !Consts.start_time in
        let s = { s with is_prover_provable } in
        (s, time_to_p)
      else (s, 0))
    (Parmap.L stats)

let split_as_true_and_false conjectures p_ctxt :
  ...
  *)
  ...
  let can_prove_conj_stats =
    lemma_provable_stats can_prove_state_stats p_ctxt
  in
