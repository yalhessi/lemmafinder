...
type goal = { hypos : string list; conc : string }
...
let remove_conjecture_dups conjectures =
  List.fold_left
    (fun acc s ->
      let is_present = List.exists (fun a -> String.equal s.body a.body) acc in
      if is_present then acc else List.append acc [ s ])
    [] conjectures
...
let update_type_table (atoms : string list) c_ctx type_tbl =
  List.iter
    (fun a ->
      ...
      try
        let typ = TypeUtils.get_type_of_atom c_ctx.env c_ctx.sigma a in
        Hashtbl.replace type_tbl a typ
      with _ -> ())
    atoms;
  type_tbl
...
let add_variable (variables : string list) (var : string) : string list =
  let var_exists =
    List.exists (fun curr_var -> String.equal curr_var var) variables
  in
  if var_exists then variables else var :: variables
...
let conjs_to_string (conjectures : conjecture list) : string =
  List.fold_left
    (fun acc conj -> acc ^ conj.conjecture_str ^ "\n")
    "" conjectures

let construct_implications (conc : string) (hyps : Sexp.t list list) : string =
  List.fold_left
    (fun acc hyp -> "(" ^ Sexp.string_of_sexpr hyp ^ "->" ^ acc ^ ")")
    conc hyps

let gen_conjecture_name prefix inc = prefix ^ "conj" ^ string_of_int (inc ())
