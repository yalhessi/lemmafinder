  ...
  let examples, _ =
    List.fold_left
      (fun (acc, index) vt ->
        ...
        ( acc ^ pipe
          ^ Consts.fmt "\"%s:\" ++ \"(\" ++ show %s ++ \")\"" n_var n_var,
          index + 1 ))
      ("", 0) var_typs
  in
  let var_string =
    List.fold_left (fun acc v -> acc ^ " " ^ v) "" (List.tl vars)
  in
  let var_typs_string =
    List.fold_left (fun acc v -> acc ^ " " ^ v) "" var_typs
  in
  let func_signature =
...
let generate_example p_ctxt typs modules current_lemma var_typs vars =
  ...
  let typs_parameter_print =
    List.fold_left
      ...
      "" typs
  in
  let start_index = String.index (List.hd var_typs) ':' + 1 in
  let end_index = String.index (List.hd var_typs) ')' in
  let hd_parameter =
    String.sub (List.hd var_typs) start_index (end_index - start_index)
  in
  let parameter_print =
  ...
  let content =
    typ_quickchick_content ^ example_print_content ^ collect_content
