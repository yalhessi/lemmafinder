...
(* Helper function to get the last variable passed to the generation definition (avoids non-printing error) *)
let break_off_last (vars : (Names.variable * Evd.econstr) list) :  (string * string list) =
  ...
  | (id,_) :: rest_rev -> 
    ...
    let rest_str = List.map (fun (x,_) -> Names.Id.to_string x) rest in
    tail_str, rest_str
...
let construct_data_collection p_ctxt = 
  ...
  let last, others = break_off_last var_types in
  ...
  func_signature examples last (String.concat " " others)
...
let get_print_signature (p_ctxt : ProofContext.proof_context) =
  let vars_rev = List.rev p_ctxt.var_types in
  let typ_str = match vars_rev with 
  | [] -> "nat"
  | (_,y) :: _ -> (Utils.get_econstr_str p_ctxt.env p_ctxt.sigma y) in
