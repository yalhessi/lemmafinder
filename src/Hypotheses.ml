...
let vars_is_subset hyp_vars g_vars : bool =
  List.fold_left
    (fun acc h_var -> acc && List.exists (fun v -> String.equal v h_var) g_vars)
    true hyp_vars
...
let subst_expr_with_value var_values hyp =
  let rec aux (acc : string list) = function
    ...
    | Expr e :: tl ->
        ...
        let to_app = "(" ^ String.concat " " str_to_concat ^ ")" in
        aux (to_app :: acc) tl
    | [] -> List.rev acc
  in
  let str_expr = aux [] hyp in
  String.concat " " str_expr

let get_invalid_hyps (p_ctxt : proof_context) (c : conjecture)
    (cgs_table : (string, string) Hashtbl.t) : t list list * t list list =
  List.fold_left
    (fun (acc_cg, acc) h ->
      let h_vars =
        Generalize_NoDup.get_variables_in_expr h [] p_ctxt.all_vars c.sigma
      in
      let is_subset = vars_is_subset h_vars c.vars in
      if is_subset then (
        (* subst with cgs to evaluate with quickchick *)
        let var_values = Hashtbl.create (List.length h_vars) in
        List.iter
          (fun h_var ->
            Hashtbl.add var_values h_var (Hashtbl.find cgs_table h_var))
          h_vars;
        let subst_hyp = subst_expr_with_value var_values h in
        let s_hyp_c =
          {
        ...
        in
        let is_valid, _ = Valid.check_validity s_hyp_c p_ctxt in
        if is_valid then (acc_cg, h :: acc) else (h :: acc_cg, acc) )
      else (acc_cg, h :: acc))
    ([], []) c.hyps
...
let construct_hyp_goal_conj invalid_hyps c =
  let hyp_goal =
    List.fold_left
      (fun acc h ->
        Sexp.normalize_sexp_vars h c.normalized_var_map ^ " -> " ^ acc)
      "" invalid_hyps
  in
  let hyp_goal =
    "forall " ^ c.vars_with_types ^ ", " ^ hyp_goal ^ " "
    ^ Sexp.string_of_sexpr c.body_sexp
  in
  let hyp_conj =
    {
      c with
      ...
      body_sexp =
        Sexp.of_string
          ("(" ^ hyp_goal ^ " " ^ Sexp.string_of_sexpr c.body_sexp ^ ")");
    }
  in
  hyp_conj
